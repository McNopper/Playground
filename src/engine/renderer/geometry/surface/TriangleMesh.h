...
private:
	...
	{
		VertexBuffer* buffer{ nullptr };   // Pointer to vertex buffer (not owned)
...
public:
	...
	// buffer: Vertex buffer containing the data (not owned by mesh)
	...
	// Set index buffer
	void setIndexBuffer(IndexBuffer* buffer, VkIndexType index_type, uint32_t count);
